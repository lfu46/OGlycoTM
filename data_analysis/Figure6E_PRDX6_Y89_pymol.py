"""
Figure 6E - PRDX6 Y89 (P30041) 3D Structure - HEK293T
O-GlcNAc site in Thioredoxin domain (5-169)

Site: Y89
  - logFC: +1.50 (significantly upregulated)
  - pLDDT: 98.25 (very high confidence - structured)
  - Domain: Thioredoxin (5-169)
  - Cell type: HEK293T

Run: /opt/homebrew/bin/pymol -c -q Figure6E_PRDX6_Y89_pymol.py
"""
...
# Domain colors
DOMAIN_COLOR = "forest"  # Green for Thioredoxin domain
...
def highlight_domain():
    ...
    # Keep pLDDT coloring, just for reference
    cmd.deselect()
...
def highlight_oglcnac_site():
    ...
    # Show side chain as spheres
    if SITE_AA == "Y":
        # Tyrosine - show phenol ring
        ...
        # Serine/Threonine - show hydroxyl
        cmd.show("spheres", f"{site_name} and (name CB or name OG or name OG1 or name CG2)")
...
def setup_view():
    ...
    # Rotate to show the site clearly
    cmd.turn("y", 30)
    cmd.turn("x", -10)
...
def render_and_save():
    ...
    # Output size: 2x2 inch at 600 DPI
    width_px = 1200
    height_px = 1200
    output_dpi = 600
