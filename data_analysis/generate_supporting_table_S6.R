...
# Generate Supporting Table S6: Identification of total proteins
#
...
process_cell_type <- function(cell_type, wp_file, protein_lengths) {
  ...
  protein_data <- wp_data %>%
    ...
    summarise(
      ...
      # Coverage = sum of lengths of unique peptides
      Coverage = sum(unique(data.frame(Peptide_Seq, Peptide_Length))$Peptide_Length),
      .groups = "drop"
    )

  # Calculate coverage properly - need to get unique peptide lengths
  protein_data <- wp_data %>%
    group_by(UniProt_Accession, Gene.Symbol, Annotation) %>%
    summarise(
      Total_Peptide = n(),
...
for (i in seq_along(cell_types)) {
  ...
  # Title text
  title_text <- paste0("Table S6", sheet_label, ". Identification of total proteins in ", cell_type, " cells")
